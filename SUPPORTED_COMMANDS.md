...
| `현재 탭 닫아줘` | Close the current Chrome tab |
| `{키워드} 탭 닫아줘` | Close the first tab whose title or URL contains the keyword |
| `close current tab` | Close the current Chrome tab |
| `close {keyword} tab` | Close the first tab whose title or URL contains the keyword |

Tab keyword matching ignores spaces. For example, `베이직 기어` can match a tab titled `베이직기어`.

Closing a tab is immediate. If the current window only has one tab, Chrome may close the window.
...
| Transcript pattern | Action |
|---|---|
| `구글에 {검색어} 검색해줘` | Search Google, reusing an existing Google tab if possible |
| `구글에서 {검색어} 검색해줘` | Search Google, reusing an existing Google tab if possible |
| `{검색어}라고 구글에 검색해줘` | Strip quote particle and search for `{검색어}` |
| `{검색어}이라 구글에서 검색해줘` | Strip quote particle and search for `{검색어}` |
| `새 탭에서 구글에 {검색어} 검색해줘` | Search Google in a new tab |
| `현재 탭에서 구글에 {검색어} 검색해줘` | Navigate the current Chrome tab to Google search |

Examples:

```bash
python main.py --dry-run "구글에 서울 날씨 검색해줘"
python main.py --dry-run "서울 날씨라고 구글에 검색해줘"
python main.py --dry-run "새 탭에서 구글에 서울 날씨 검색해줘"
python main.py --dry-run "현재 탭에서 구글에 서울 날씨 검색해줘"
```
...
| `search google for macos automation` | Search Google |
| `google python unittest` | Search Google |
